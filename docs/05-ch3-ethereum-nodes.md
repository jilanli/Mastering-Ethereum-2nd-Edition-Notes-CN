...
> [!Note]
> 这些规范（Specs）并非旨在成为全节点实现，而是作为一种“可执行的伪代码规范”存在。
...
> **全节点 (Full nodes)**
>
> 定期裁剪（Discard）历史状态和收据的以太坊节点——这通常是你启动节点时的默认选项。
...
请不要将以太坊中的“远程客户端”概念与“轻客户端”（Light Client）混淆。轻客户端类似于比特币中的简单支付验证（SPV）客户端。轻客户端通过验证区块头（Block Headers）并使用默克尔证明（Merkle Proofs）来验证交易是否包含在区块链中及其产生的影响，这使得它们拥有与全节点相近的安全级别。

相反，以太坊远程客户端不验证区块头或交易。它们完全信任全节点所提供的区块链访问权限，因此丧失了显著的安全性和匿名性保障。你可以通过使用自己运行的全节点来缓解这些问题。
...
本节涵盖了 `Geth-Prysm` 和 `Reth-Lighthouse` 客户端软件。它还假设你使用的是类 **Unix** 命令行环境。示例显示了在运行 **Bash shell**（命令行执行环境）的 **macOS** 上的命令和输出。这些说明在大多数 **Linux** 发行版上无需修改即可运行。**Windows** 用户可以使用Windows Subsystem for Linux (WSL2)。
...
你还需要安装 [Go](https://golang.org/) 和 [Rust](https://www.rust-lang.org/)。你可以查看它们的官方网站以获取安装指南。
...
**克隆存储库**。第一步是克隆Git存储库以获取源代码副本。在 `execution` 子文件夹中使用如下 `git` 命令在你本地克隆所选存储库：
...
现在轮到共识客户端了。**Prysm** 是共识规范的 **Go** 语言实现，由 **Offchain Labs** 积极开发。起初，它是合并（The Merge）后使用最广泛的共识客户端。现在，得益于社区提升客户端多样性的巨大努力，它的市场份额大幅下降，目前维持在37%。
...
现在，你已经拥有了一个正在向区块链最顶端（tip of the chain）同步的以太坊全节点。请注意，同步过程可能会耗费大量时间（根据你的硬件性能和互联网连接速度，可能需要数小时甚至数天）。

[!Note] 如果你想深入了解本示例中使用的具体命令和 CLI 标志（flags），[Geth](https://oreil.ly/zYviP) 和 [Prysm](https://oreil.ly/4sn6-) 的官方文档是最佳查阅途径。
...
> [!Note]
> 如果你想详细了解本示例中使用的具体命令和 **CLI** 标志（flags），[Reth](https://reth.rs/) 和 [Lighthouse](https://oreil.ly/vEghS) 的官方文档是最佳查阅地点。
...
> 
> 这里有一个完美的解决方案：**BuidlGuidl Client** 项目。他们创建了一个单行命令，让你能够一键运行以太坊节点。不相信？去[亲自看看](https://oreil.ly/9FKZd)吧。
> 
> 另一个选择是使用 [Dappnode](https://dappnode.com/)。你可以选择两种不同的方案：
> * 购买一个内置以太坊全节点的即插即用（plug-n-play）设备。
> * 安装 **Dappnode Core** 软件，它可以让你非常轻松地启动以太坊全节点。
