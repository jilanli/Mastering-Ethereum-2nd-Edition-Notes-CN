...
虽然命令式编程更常被程序员使用，但编写完全按预期执行的程序可能非常困难[^1]。程序的任何部分改变任何其他部分状态的能力使得推导程序的执行变得困难，并引入了许多产生漏洞的机会。相比之下，声明式编程更容易理解程序的行为：由于它没有副作用，程序的任何部分都可以被隔离理解。
...
[^1]: 我自己觉得在编写业务逻辑时候应该尽量减小代码的副作用。虽然无法像Lisp那样彻底的函数式编程，但是起码应该将副作用集中管理，核心逻辑保持纯净，能显著提升代码质量、可测性和并发安全性。
